...
# Check if enabled
ENABLED=$(tmux show-option -gv @monochrome-night-tmux_show_netspeed 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
INTERFACE=$(tmux show-option -gv @monochrome-night-tmux_netspeed_iface 2>/dev/null)
# Show IP address
SHOW_IP=$(tmux show-option -gv @monochrome-night-tmux_netspeed_showip 2>/dev/null)
...
if [[ -z $INTERFACE ]]; then
  ...
  tmux set-option -g @monochrome-night-tmux_netspeed_iface "$INTERFACE"
fi
